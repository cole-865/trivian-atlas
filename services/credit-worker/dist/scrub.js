...
export function scrubPII(input) {
    ...
    t = t.replace(/(\bBDS\b\s*[-:]\s*)(\d{1,2}\/\d{1,2}\/\d{2,4})/gi, "$1**/**/****");
    // "DOB 05/14/1987", "DOB:05/14/1987", "DOB- 05/14/1987"
    t = t.replace(/(\bDOB\b\s*[:\-]?\s*)(\d{1,2}\/\d{1,2}\/\d{2,4})/gi, "$1**/**/****");
    // "DATE OF BIRTH: 05/14/1987"
    t = t.replace(/(\bDATE\s+OF\s+BIRTH\b\s*[:\-]?\s*)(\d{1,2}\/\d{1,2}\/\d{2,4})/gi, "$1**/**/****");
    ...
    t = t.replace(/\b(\+?1[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}\b/g, "***-***-****");
    // -----------------------------
    // Email addresses
    // -----------------------------
