...
function applyMaxTier(tier, maxTier) {
...
function addressesMatch(primaryAddress, coApplicantAddress) {
...
function calculateApplicantMovement(applicant) {
    let movement = 0;
    const scoreFactors = [];
...
function monthsSinceDate(value, now = new Date()) {
    if (!value)
        return null;
...
function calculateJobStabilityMovement(applicant) {
...
function hasHeavyCollectionsOrChargeoffs(applicant) {
...
function hasNoAutoWeakDepth(applicant) {
    const totalTradelines = countOrNull(applicant.totalTradelines);
...
function hasCleanRebuildAfterPublicRecord(applicant) {
    const totalTradelines = countOrNull(applicant.totalTradelines);
    const openTradelines = countOrNull(applicant.openTradelines);
    return (!hasHeavyCollectionsOrChargeoffs(applicant) &&
...
function hasPublicRecord(applicant) {
    return (countOrNull(applicant.monthsSinceBankruptcy) !== null ||
        (countOrNull(applicant.bankruptcyCount) ?? 0) > 0 ||
        (countOrNull(applicant.publicRecordCount) ?? 0) > 0);
}
function hasCleanCurrentTrades(applicant) {
    const openTradelines = countOrNull(applicant.openTradelines);
    const pastDue = countOrNull(applicant.pastDueAmount);
...
function hasCleanBehaviorAfterPublicRecord(applicant) {
    return hasCleanRebuildAfterPublicRecord(applicant) && hasCleanCurrentTrades(applicant);
}
function calculateBehaviorModifiers(applicant) {
    let movement = 0;
    const scoreFactors = [];
    ...
    if (monthsSincePublicRecord !== null &&
...
function getTierCapFactors(applicant) {
    const factors = [];
    const monthsSincePublicRecord = countOrNull(applicant.monthsSinceBankruptcy);
    const applicantHasPublicRecord = hasPublicRecord(applicant);
    const hasUndatedBankruptcy = (countOrNull(applicant.bankruptcyCount) ?? 0) > 0 &&
    ...
    else if (monthsSincePublicRecord !== null &&
...
function applyTierCaps(tier, factors) {
...
function getCoApplicantSkipFactors(args) {
    const factors = [];
...
export function scoreDealTier(args) {
    if (!args.primary) {
        return {
            decision: "denied",
            tier: null,
    ...
    if (coApplicantIsValid && args.coApplicant) {
        ...
        scoreFactors.push(...primaryJobStability.scoreFactors, ...coJobStability.scoreFactors);
    }
    else {
