...
with latest_snapshot as (
  select distinct on (snapshot.organization_id, snapshot.deal_number)
    ...
    coalesce(
      case when snapshot.remaining_payment > 0 then snapshot.remaining_payment end,
      case when snapshot.previous_payment_amount > 0 then snapshot.previous_payment_amount end,
      case when snapshot.final_payment_amount > 0 then snapshot.final_payment_amount end,
      case when snapshot.total_payment_amount > 0 then snapshot.total_payment_amount end
...
comment on column public.account_payment_signals.scheduled_payment_amount
  is 'Estimated recurring scheduled installment from latest All Accounts snapshot, preferring remaining payment, previous payment, final payment, then total payment amount.';

comment on column public.account_payment_signals.catchup_gap_estimated
  is 'Estimated 60-day expected scheduled installments minus positive non-reversal payments received in the last 60 days.';
