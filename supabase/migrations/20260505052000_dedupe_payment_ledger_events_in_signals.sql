...
ledger_events as (
  select distinct on (
    ...
    ledger.principal_applied_amt,
    ledger.interest_applied_amt,
    ledger.late_fees_applied_amt,
    ledger.other_fees_applied_amt,
    ledger.side_note_applied_amt,
    ledger.down_applied_amt,
    ledger.credit_applied_amt,
    ledger.balance_amount,
    ledger.due_amount,
    ledger.days_late,
    ledger.late_fee_amount,
    ledger.principal_due_amount,
    ledger.interest_due_amount,
    ledger.other_fees_due_amount,
    ledger.side_note_due_amount,
    ledger.processing_fee_due_amount
  )
    ledger.*
...
payment_rollups as (
  select
    ...
    greatest(
      ...
      - coalesce(sum(payments.reversal_amount) filter (
        where payments.paid_date >= current_date - interval '30 days'
      ), 0),
      0
    ) as total_paid_30d,
    ...
    greatest(
      coalesce(sum(payments.positive_amount), 0)
      - coalesce(sum(payments.reversal_amount), 0),
      0
    ) as lifetime_paid_from_ledger,
...
comment on view public.account_payment_signals
  is 'Sanitized per-account payment performance signals built from latest account snapshots and deduped, net positive ledger payments after reversals/returns.';

comment on column public.account_payment_signals.total_paid_60d
  is 'Net positive non-reversal payments in the last 60 days after deduping identical ledger events and subtracting reversal/return amounts.';
