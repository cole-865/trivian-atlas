...
comment on view public.account_payment_signals
  is 'Sanitized per-account payment performance signals built from latest account snapshots and net positive ledger payments after reversals/returns.';

comment on column public.account_payment_signals.total_paid_60d
  is 'Net positive non-reversal payments in the last 60 days after subtracting reversal/return amounts.';

comment on column public.account_payment_signals.payment_ratio_60d
  is 'Net positive 60-day payments after reversals divided by expected scheduled payments in the last 60 days.';
