...
select
  ...
  coalesce(expected.expected_paid_60d, 0) - coalesce(rollup.total_paid_60d, 0) as catchup_gap_estimated,
  case
...
comment on column public.account_payment_signals.catchup_gap_estimated
  is 'Signed 60-day payment shortfall: expected scheduled payments for the last 60 days minus deduped positive non-reversal payments. Negative values mean the account paid more than the recent scheduled amount.';
