...
alter table public.dms_accounts_snapshot
  add column if not exists monthly_payment_amount numeric null;

comment on column public.dms_accounts_snapshot.monthly_payment_amount
  is 'Explicit scheduled payment amount from the All Accounts Monthly Payment column. The DMS label says monthly, but values represent the scheduled installment for the account frequency.';

update public.dms_accounts_snapshot
set monthly_payment_amount = public.dms_parse_numeric(raw_data ->> 'Monthly Payment')
where monthly_payment_amount is null
  and raw_data ? 'Monthly Payment';
...
comment on column public.account_payment_signals.scheduled_payment_amount
  is 'Estimated recurring scheduled installment for payment ratios. Prefers the explicit All Accounts Monthly Payment column, then falls back to prior compatible payment amount fields.';
